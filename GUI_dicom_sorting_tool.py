...
class DicomSortingGUI(QWidget):
    ...
    def execute_sorting(self):
        ...
        try:
            # Update the call to sort_dicom to reflect the correct anonymization flags
            sort_dicom(input_dir, output_dir, basic_anonymize, id_map, decompress, strict_anonymize, skip_derived, skip_burned)
            QMessageBox.information(self, "Success", "DICOM sorting completed successfully.")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"An error occurred during sorting: {str(e)}")
