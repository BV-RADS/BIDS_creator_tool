...
def find_dicom_files(directory, read_all):
    ...
    for root, dirs, files in os.walk(directory):
        for f in files:
            ...
            logging.info(f"Found potential file: {file_path}")
    
    logging.info(f"Total potential files found: {len(dicom_files)}")
    if not read_all:
        dicom_files = dicom_files[:5]
        logging.info(f"Limiting to first 5 files: {dicom_files}")
    return dicom_files
    ...
    try:
        ...
        for field in fields:
            ...
            if tag in dicom:
                ...
                info[field] = ''
        logging.info(f"Successfully extracted info from {file_path}")
        return info
...
def main(dicom_dir, output_path, read_all=False):
    ...
    logging.info(f"Starting script. Directory: {dicom_dir}, Output: {output_path}, Read all: {read_all}")
    ...
    for file_path in tqdm(all_files, desc="Processing DICOM files", ncols=100):
        ...
        if info:
            ...
            # If any of these fields are different, consider it a unique sequence
            if unique_id not in unique_sequences and any(unique_id):
                unique_sequences[unique_id] = info
                logging.info(f"Added new unique sequence. Total: {len(unique_sequences)}")

    logging.info(f"Writing {len(unique_sequences)} sequences to {output_path}")
